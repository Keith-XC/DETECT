...
@click.option(
    "--rows", "row_seeds", type=num_range, help="Random seeds to use for image rows", required=True
)
@click.option(
    "--cols",
    "col_seeds",
...
def generate_style_mix(
    ...
    all_images = (
        (all_images.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()
    )
