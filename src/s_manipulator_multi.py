...
class MulticlassManipulatorSSpace(BaseManipulatorSSpace):
    ...
    def bisection_factor_adjustment(
        ...
        while high - low > tolerance and iterations < max_iterations:
            ...
            original_class = np.argmax(
                self.classifier(
                    self.preprocess_fn(
                        generate_image_with_s_latents(
                            self.generator.synthesis, s_gradients
                        ).squeeze(0)
                    ).unsqueeze(0)
                )
                .squeeze(0)
    ...
    def constrained_hill_climbing_factor_adjustment(
        ...
        if not is_misclassified(initial_predicted_class, original_predicted_class):
            print(
                f"Warning: Initial condition does not satisfy misclassification! {initial_predicted_class} == {original_predicted_class}"
            )
            ...
            for candidate_factor in candidates:
                ...
                if is_valid and decision_margin < best_decision_margin:
                    ...
                    if verbose:
                        ...
                        print(
                            f"Hill climbing iteration {iteration}: factor={factor:.3f}, "
    ...
    def compare_perturbed(
        ...
        extent = (-extent_factor) * np.sign(
            gradient_single
        )  # np.sign(prediction_target) *is always decreasing
        ...
        prediction_perturbed = (
            self.classifier(self.preprocess_fn(img_perturbed_tensor.squeeze(0)).unsqueeze(0))
            .squeeze(0)
            .cpu()
            .detach()
            .numpy()
        )
        adjusted_confidence, predicted_class, predicted_top_confidence = self._get_target_value(
            prediction_perturbed
        )
            ...
            for layer_name, rank_data in ranked_gradient_info.items():
                ...
                for top_n in range(len(rank_data["ranked_indices"])):
                    ...
                    if oracle == "confidence_drop" and self._evaluate_oracle(
                        ...
                        if img_perturbed is not None and predicted_class != self.target_logit:  #
                            save_flag = True
