...
class BinaryManipulatorSSpace(BaseManipulatorSSpace):
    ...
    def compare_perturbed(
        self, s_gradients, layer_name, rank_data, prediction_target, extent_factor=40, top_n=0
    ):
        """
        ...
        prediction_perturbed = self.classifier(
            self.preprocess_fn(img_perturbed_tensor.squeeze(0)).unsqueeze(0)
        ).squeeze(0)
    ...
    def bisection_factor_adjustment(
        self,
        current_factor,
        initial_confidence,
        current_confidence,
        ...
        while high - low > tolerance and iterations < max_iterations:
            ...
            print(
                f"low: {low:.2f}, high: {high:.2f}, mid: {mid:.2f}, adjusted_confidence: {adjusted_confidence:.2f}, "
    ...
    def constrained_hill_climbing_factor_adjustment(
        ...
        for iteration in range(max_iterations):
            ...
            for candidate_factor in candidates:
                is_valid, confidence_drop, img_perturbed, adjusted_confidence = evaluate_factor(
                    candidate_factor
                )
                ...
                if is_valid and abs(adjusted_confidence) < abs(best_adjusted_confidence):
                    ...
                    no_improvement_count = 0
                    print(
                        f"Constrained hill climbing iteration {iteration}: factor={factor:.3f}, "
    ...
    def handle_one_seed(
        ...
        img_tensor = self.generator(
            z, label_sg, truncation_psi=truncation_psi, noise_mode="const"
        ...
        prediction = self.classifier(
            self.preprocess_fn(img_tensor.squeeze(0)).unsqueeze(0)
        ).squeeze(0)
                ...
                for top_n in range(len(rank_data["ranked_indices"])):
                    ...
                    if oracle == "confidence_drop" and self._evaluate_oracle(
                        ...
                        if self.segmenter is not None:
                            ...
                            seg_result = self.segmenter.detect_changes(
                                img, img_perturbed, mask, mask_perturbed
                            )
                        else:
                            seg_result = None
                        ...
                        (
                            best_factor,
                            best_adjusted_confidence,
                            best_img_perturbed,
                            best_confidence_drop,
                        ...
                        if (
                            best_img_perturbed is not None
                            and best_adjusted_confidence * prediction_target < 0
                        ):
